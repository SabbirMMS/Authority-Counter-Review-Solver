...
<head>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .status {{ display: inline-flex; align-items: center; padding: 7px 12px; border-radius: 999px; color:
        #fff; font-weight: 700; letter-spacing: 0.03em; background: {severity_color}; }}
    ...
    .file-card {{ margin-bottom: 14px; border: 1px solid var(--line); border-radius: 14px; overflow: hidden; background:
        #fff; }}
    ...
    .violations-table th, .violations-table td {{ padding: 10px; border-bottom: 1px solid var(--line); text-align: left;
        vertical-align: top; font-size: 13px; }}
    .violations-table th {{ background:
        #f9fcff; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em; color: #335475; }}
    .violations-table tr:last-child td {{ border-bottom: none; }}
    .line-chip {{ display: inline-block; padding: 2px 8px; border-radius: 999px; background:
        #eef5ff; border: 1px solid #cadef4; font-weight: 700; }}
...
<body>
    ...
    <div class = \"card\">
        ...
        <div class = \"grid\">
        <div class =
            \"metric\"><div class=\"label\">Branch</div><div class=\"value\">{escape(result.branch)}</div></div>
        ...
        <div class =
            \"metric\"><div class=\"label\">Checked Files</div><div class=\"value\">{result.checked_files}</div></div>
        <div class =
            \"metric\"><div class=\"label\">Skipped Files</div><div class=\"value\">{result.skipped_files}</div></div>
        <div class =
            \"metric\"><div class=\"label\">Violations</div><div class=\"value\">{len(result.violations)}</div></div>
    </div>
